...
from email_at_rest import display_email, hash_email
...
def _find_user(cur, ident: str):
    ...
    cur.execute("SELECT id, username, email, email_encrypted, is_admin FROM users WHERE email = %s OR email_hash = %s;", (ident, hash_email(ident)))
    return cur.fetchone()
...
def cmd_grant(conn, ident: str, role: str, create_role: bool) -> int:
    with conn.cursor() as cur:
        ...
        if not user:
            ...
            return 1
        user_id, username, email, email_encrypted, is_admin = user
    ...
    conn.commit()
    print(f"✅ Granted '{role}' to {username}{' <'+display_email(email, email_encrypted)+'>' if display_email(email, email_encrypted) else ''}")
    return 0
...
def cmd_revoke(conn, ident: str, role: str) -> int:
    ...
    conn.commit()
    print(f"✅ Revoked '{role}' from {username}{' <'+display_email(email, email_encrypted)+'>' if display_email(email, email_encrypted) else ''}")
    return 0
...
def cmd_status(conn, ident: str) -> int:
    ...
    print(f"User: {username}{' <'+display_email(email, email_encrypted)+'>' if display_email(email, email_encrypted) else ''} (id={user_id})")
...
def cmd_list(conn) -> int:
    ...
    for (uid, username, email, email_encrypted, is_admin) in rows:
        print(f"- {username}{' <'+display_email(email, email_encrypted)+'>' if display_email(email, email_encrypted) else ''} (id={uid})")
    return 0
